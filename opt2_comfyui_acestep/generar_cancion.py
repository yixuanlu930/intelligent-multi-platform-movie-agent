...
Uso:
    ...
    # Cambia el prompt y la letra:
    python generar_cancion.py \\
        --tags "synthwave, retro, 120 bpm, neon, dreamy" \\
        --lyrics-file mi_letra.txt

    # Genera el instrumental:
    python generar_cancion.py --workflow acestep_workflow_instrumental.json

    # Cambia la semilla y la duración:
    python generar_cancion.py --seed 7 --seconds 45

Requisitos:
    pip install requests websocket-client
    ComfyUI corriendo en http://localhost:8188 con el nodo Ace Step instalado.
"""
...
def aplicar_overrides(
    ...
    Los nodos están identificados por su class_type (no por su id, que puede
    cambiar si alguien reordena el workflow).
    """
    # En el workflow oficial de Ace Step solo hay UN TextEncodeAceStepAudio
    # (el negativo se genera con ConditioningZeroOut a partir del positivo).
    for node in workflow.values():
        ct = node.get("class_type")
        if ct == "TextEncodeAceStepAudio":
            if tags is not None:
                node["inputs"]["tags"] = tags
            if lyrics is not None:
                node["inputs"]["lyrics"] = lyrics
        if ct == "KSampler":
            if seed is not None:
                node["inputs"]["seed"] = seed
            if steps is not None:
                node["inputs"]["steps"] = steps
        if ct == "EmptyAceStepLatentAudio" and seconds is not None:
            node["inputs"]["seconds"] = seconds


def encolar(workflow: dict, server: str, client_id: str) -> str:
...
def esperar_finalizacion(server: str, client_id: str, prompt_id: str,
    ...
    """Se conecta al WebSocket de ComfyUI y espera al evento `executing` con
    `node == null`, que indica fin de la ejecución. Devuelve el history."""
...
def descargar_audio(server: str, history: dict, destino: Path) -> Path | None:
    ...
    for node_outputs in outputs.values():
        for clave, items in node_outputs.items():
            ...
            for item in items:
                ...
                with urllib.request.urlopen(url) as src, open(destino, "wb") as dst:
                    dst.write(src.read())
                return destino
    return None
...
def main() -> int:
    ...
    parser.add_argument("--workflow", "-w", type=Path,
    ...
    parser.add_argument("--lyrics-file", type=Path,
                        help="Lee la letra desde un fichero.")
    ...
    parser.add_argument("--output", "-o", type=Path, default=Path("cancion_generada.flac"),
