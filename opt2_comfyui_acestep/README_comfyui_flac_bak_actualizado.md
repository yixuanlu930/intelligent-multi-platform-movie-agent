...
## Explicacion de los archivos `.flac`

Los archivos `.flac` son los audios generados por ComfyUI/Ace Step durante las pruebas reales de la practica.

FLAC significa **Free Lossless Audio Codec**. Es un formato de audio comprimido sin perdida, es decir, mantiene la calidad del audio generado mejor que formatos con perdida como MP3. ComfyUI utiliza este formato para guardar el resultado final del nodo `SaveAudio`.

En esta practica pueden aparecer estos archivos:
...
Este archivo es la evidencia principal de ejecucion. Se genero a partir del workflow:
...
En la prueba realizada, el archivo tenia un tamano aproximado de:

```text
843 KB
```

Este audio demuestra que:

```text
ComfyUI estaba corriendo correctamente
El modelo Ace Step fue cargado correctamente
El workflow se ejecuto sin errores
El nodo SaveAudio genero un archivo de salida real
El audio pudo descargarse y reproducirse en macOS
```

### `smoke_test_2.flac`

Este archivo es una segunda prueba de ejecucion. Puede generarse despues de corregir `generar_cancion.py`, para verificar que el cliente Python ya espera correctamente a ComfyUI y descarga el audio automaticamente sin quedarse bloqueado por el timeout del WebSocket.

Su funcion es validar la version corregida del script.

### Relacion con ComfyUI

Cuando se ejecuta el workflow desde la interfaz web, ComfyUI guarda internamente el audio en su carpeta de salida:

```text
ComfyUI/output/
```

Por ejemplo, ComfyUI genero:

```text
ace_step_instrumental_00001_.flac
```

Despues, el cliente Python lo descarga y lo copia en la carpeta de la practica con el nombre indicado en `--output`, por ejemplo:

```text
smoke_test.flac
```

Tambien se puede descargar manualmente con la API de ComfyUI:

```bash
curl -L "http://127.0.0.1:8188/view?filename=ace_step_instrumental_00001_.flac&type=output" \
  -o smoke_test.flac
```

### Son necesarios para ejecutar la practica?

No son necesarios para ejecutar el codigo. Los archivos `.flac` son resultados de ejecucion, no forman parte del codigo fuente.

La practica puede ejecutarse con:
...
Sin embargo, es recomendable entregar al menos:

```text
smoke_test.flac
```

como evidencia de que la generacion real funciono.

### Puedo borrarlos?

Si solo se quiere conservar el codigo fuente, se pueden borrar los `.flac` porque son salidas generadas.

Pero para una entrega universitaria es recomendable conservar:

```text
smoke_test.flac
```

y borrar solo duplicados o pruebas adicionales, por ejemplo:

```text
smoke_test_2.flac
```

si no se quiere entregar mas de una evidencia.

Resumen:

```text
smoke_test.flac      recomendable conservar como evidencia
smoke_test_2.flac    opcional, se puede borrar si es duplicado
*.flac               resultados de ejecucion, no codigo fuente
```


---

## Explicacion del archivo `.bak`

El archivo:

```text
generar_cancion.py.bak
```

es una **copia de seguridad temporal** del script `generar_cancion.py`.

Se creo durante la correccion del cliente Python, antes de modificar el codigo para solucionar el problema del timeout del WebSocket. Su objetivo era poder restaurar la version anterior si el parche fallaba.

En esta practica, el archivo `.bak` no es codigo principal ni es necesario para ejecutar el workflow.

### Para que sirve?

Sirve solo como respaldo:

```text
generar_cancion.py      version activa que se ejecuta
generar_cancion.py.bak  copia antigua de seguridad
```

Si `generar_cancion.py` se rompiera durante una edicion manual, se podria restaurar con:

```bash
cp generar_cancion.py.bak generar_cancion.py
```

### Es necesario para la entrega?

No. Para la entrega final debe conservarse la version corregida:
...
porque no aporta funcionalidad nueva y puede confundir al corrector.

### Puedo borrarlo?

Si `generar_cancion.py` ya esta corregido y comprobado, si. Se puede borrar con:

```bash
rm generar_cancion.py.bak
```

Resumen:

```text
generar_cancion.py      conservar, es el script principal
generar_cancion.py.bak  borrar, es una copia temporal
*.bak                   no entregar salvo que se pida expresamente
```
...
*.bak             copias de seguridad temporales, por ejemplo generar_cancion.py.bak
